...
const searchPoint = js.indexOf(funcStart);
console.log("Found function start at", searchPoint);

if (searchPoint > -1) {
    const endPoint = js.indexOf('/* ============================================================', searchPoint);
    console.log("Found next boundary at", endPoint);
    
    if (endPoint > searchPoint) {
        // Just extract the slice exactly and test its length
        const slice = js.substring(searchPoint, endPoint);
        console.log("Slice length is", slice.length);
