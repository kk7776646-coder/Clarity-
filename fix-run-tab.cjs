...
const funcStart = 'function renderRunTestTab(container, projectId, analysis) {';
const startIndex = js.indexOf(funcStart);

if (startIndex > -1) {
    const endOfFunction = js.indexOf('// -----------------------------------------------------------------------------', startIndex);
    if (endOfFunction > -1) {
        
        // This is safe to just replace the whole function if we don't have trailing functions before the next line
        const oldCode = js.substring(startIndex, endOfFunction);
        
        const newCode = `function renderRunTestTab(container, projectId, analysis) {
  ...
  function getTerminalColorClasses(str) {
      if (!str) return "";
      // Strip ANSI escape codes safely for HTML view
      // More robust regex to handle multiple ANSI formats
  ...
  async function spawnTerminal(command, title) {
      try {
          termOut.innerHTML = ''; // optimistically clear
          const res = await window.Clarity.api.request('/api/projects/' + projectId + '/terminals', {
              method: 'POST',
  ...
  // Derive initial command automatically based on frameworks detected if possible, fallback to standard scripts
  ...
  if (analysis.frameworks && analysis.frameworks.length > 0) {
      if (analysis.primaryLanguage === "Python") {
          defaultRunCmd = "python app.py"; // basic fallback
      }
  }
  ...
  // Track scrolling to disable auto-scroll if user scrolled up manually
...
`;
        ...
        console.log("Success replacing renderRunTestTab!");
    } else {
