...
const regex = /exportPdfBtn\?\.addEventListener\("click", async \(\) => \{[\s\S]*?\}\);\s*\}\s*if/m;
...
const newBlock = `exportPdfBtn?.addEventListener("click", async () => {
      ...
      try {
          ...
          header.innerText = "Project Chat Export - " + dateStr;
          ...
          const opt = {
            margin:       10, // mm
            filename:     'Clarity_Project_Chat_Export.pdf',
...
if (regex.test(js)) {
   ...
   console.log("Successfully replaced export logic in project.js");
} else {
