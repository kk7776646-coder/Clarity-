...
const targetStr = `  let runInterval = null;
...
if (js.includes(targetStr)) {
    const endStr = `  startPolling();
}`;
    const startIndex = js.indexOf(targetStr);
    const endIndex = js.indexOf(endStr, startIndex) + endStr.length - 1; // before the closing bracket of the function
    
    if (startIndex > -1 && endIndex > -1) {
        const fullLogicBlock = js.substring(startIndex, endIndex);
...
`;
        ...
        js = js.replace(fullLogicBlock, newLogicStr);
        fs.writeFileSync('js/pages/project.js', js, 'utf8');
        console.log("Success string replacement.")
    } else {
        console.log("Failed to find end marker for Logic block.");
    }
} else {
    console.log("Target Logic block not found.");
}
