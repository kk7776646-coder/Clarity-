...
const targetStr = `function renderRunTestTab(container, projectId, analysis) {`;
const targetEnd = `// -----------------------------------------------------------------------------
// Utilities`; // Let's check what's right after it.

const idx = js.indexOf(targetStr);
if (idx > -1) {
    const nextLine = js.indexOf('function fileTypeIcon(ext) {', idx);
    if (nextLine > -1) {
        // We need to walk backwards to find the end of the previous function or comment block
        const commentBlock = js.lastIndexOf('/* ============================================================', nextLine);
        
        if (commentBlock > idx) {
             const oldCode = js.substring(idx, commentBlock);
             
             // Now replace oldCode with our new function code
...
`;
             ...
             console.log("Success replacing renderRunTestTab function strictly backwards!");
        }
    }
} else {
